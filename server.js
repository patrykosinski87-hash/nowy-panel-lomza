...
app.post('/api/register', async (req, res) => {
    ...
    db.get("SELECT COUNT(*) as count FROM staff", (err, row) => {
        ...
        db.run("INSERT INTO staff (username, password, role) VALUES (?, ?, ?)", [username, hash, role], () => res.json({ success: true }));
    });
});
...
app.get('/api/data', protect, (req, res) => {
    db.all("SELECT * FROM players", (err, players) => {
        db.all("SELECT id, username, role FROM staff", (err, staff) => {
            db.all("SELECT * FROM logs ORDER BY time DESC LIMIT 10", (err, logs) => {
...
app.post('/api/staff/delete', protect, (req, res) => {
    if(req.user.role !== 'Owner') return res.status(403).json({ error: 'Brak uprawnień' });
    db.run("DELETE FROM staff WHERE id = ?", [req.body.id], () => res.json({ success: true }));
});

app.listen(PORT, () => console.log(`🚀 TX-ELITE PRO: http://localhost:${PORT}`));